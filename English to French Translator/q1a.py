...
encoder_outputs, forward_state_h, forward_state_c, backward_state_h, backward_state_c = encoder(encoder_inputs)
encoder_states = [forward_state_h, forward_state_c, backward_state_h, backward_state_c]

# concat the states for the LSTM decoder
...
decoder_outputs, idk_state_h, idk_state_c = decoder_lstm(decoder_inputs, initial_state=encoder_states)
...
end_time = time.time()
print(f"Total time to train model: {end_time-start_time}s")

# Save model
model.save("Architecture1")
model.save("Architecture1.h5")  # additional format due to noted issues with non-h5 persistence

# Plotting the training and validation metrics
plot_graphs(history, 'accuracy', 'Architecture 1 Accuracy')
plot_graphs(history, 'loss', 'Architecture 1 Loss')
plot_graphs(history, 'mean_squared_error', 'Architecture 1 MSE')
plot_graphs(history, 'f1_m', 'Architecture 1 Score')
plot_graphs(history, 'precision_m', 'Architecture 1 Precision')
plot_graphs(history, 'recall_m', 'Architecture 1 Recall')
...
decoder_state_input_h = keras.Input(shape=(latent_dim*2,))  # potentially change
...
# To predict the next word in the sequence, set the initial states to the states from the prev timestep
decoder_outputs, state_h_dec, state_c_dec = decoder_lstm(decoder_inputs, initial_state=decoder_hidden_state_input)
...
for seq_index in rand_indices:
    ...
    if 'Jump' in input_texts[seq_index]:
        ...
        references = ['Cours !', 'Courez !', 'Prenez vos jambes à vos cous !', 'File !', 'Filez !', 'Cours !',
                      'Fuyez !', 'Fuyons !']
