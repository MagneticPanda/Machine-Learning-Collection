...
encoder_lstm1 = keras.layers.LSTM(latent_dim, return_state=True, return_sequences=True)
encoder_outputs_1, state_h1, state_c1 = encoder_lstm1(encoder_inputs)

# LSTM Layer 2
encoder_lstm2 = LSTM(latent_dim, return_state=True)
encoder_outputs_2, state_h2, state_c2 = encoder_lstm2(encoder_outputs_1)
...
decoder_outputs, decoder_forward_state, decoder_backward_state = decoder_lstm(decoder_inputs, initial_state=encoder_states)
...
# Save model
model.save("Architecture3")
model.save("Architecture3.h5")  # additional format due to noted issues with non-h5 persistence

# Plotting the training and validation metrics
plot_graphs(history, 'accuracy', 'Architecture 3 Accuracy')
plot_graphs(history, 'loss', 'Architecture 3 Loss')
plot_graphs(history, 'mean_squared_error', 'Architecture 3 MSE')
plot_graphs(history, 'f1_m', 'Architecture 3 Score')
plot_graphs(history, 'precision_m', 'Architecture 3 Precision')
plot_graphs(history, 'recall_m', 'Architecture 3 Recall')
